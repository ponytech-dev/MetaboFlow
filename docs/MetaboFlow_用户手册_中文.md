...
| **`FILTER_NONSPECIFIC`** | **`TRUE`** | **非特异性通路过滤开关。默认开启，设为 `FALSE` 可关闭** |

---

### ⚠️ 非特异性通路自动过滤（重要功能）

> **设计理由：** 在非靶向代谢组学中，某些超大/泛代谢通路（如 "Metabolic pathways"、"ABC transporters"）在**几乎任何实验条件下**都会被标记为显著，因为它们包含大量代谢物，命中概率极高。这类结果**不具有生物学特异性**，直接用于论文会导致审稿人质疑通路分析的可信度。

**默认行为（`FILTER_NONSPECIFIC = TRUE`）：**

MetaboFlow 自动过滤以下非特异性通路：

| 过滤类型 | 规则 | 示例 |
|----------|------|------|
| **关键词黑名单** | 通路名包含以下任一关键词 | Metabolic pathways、Biosynthesis of secondary metabolites、Biosynthesis of amino acids、Carbon metabolism、2-Oxocarboxylic acid metabolism、Biosynthesis of cofactors、ABC transporters、Protein digestion and absorption、Mineral absorption、Aminoacyl-tRNA biosynthesis |
| **通路大小过滤** | 通路内代谢物总数 > 150 | 自动排除含 150+ 代谢物的超大通路 |

**输出方式（双文件保证完整性）：**

| 文件 | 内容 | 用途 |
|------|------|------|
| `*.xlsx` | **全部**显著通路（未过滤） | 完整记录，供补充材料使用 |
| `*_filtered.xlsx` | 过滤后的特异性通路 | **用于论文正文图表**（默认绘图数据源） |

**关闭过滤：**

如果您的实验确实需要保留这些泛通路（例如研究整体代谢重编程），可在参数区设置：

```r
FILTER_NONSPECIFIC <- FALSE   # 关闭非特异性过滤，保留所有显著通路
```

> **实测效果（MethiocarbB 数据集）：** WF3 KEGG 从 52 条显著通路 → 46 条（过滤了 6 条泛代谢通路），WF1/WF2/WF4 的 SMPDB 通路本身较为特异，几乎无需过滤。
